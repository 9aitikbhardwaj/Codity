...
const WORKER_ID = `worker-${os_1.default.hostname()}-${process.pid}-${(0, crypto_1.randomUUID)().substring(0, 8)}`;
...
async function registerWorker() {
    await (0, database_1.query)(`INSERT INTO workers (id, hostname, ip_address, status, concurrency_slots, last_heartbeat_at)
     VALUES (?, ?, ?, 'active', ?, NOW())
     ON DUPLICATE KEY UPDATE status='active', last_heartbeat_at=NOW(), concurrency_slots=?`, [WORKER_ID, HOSTNAME, IP_ADDRESS, CONCURRENCY_SLOTS, CONCURRENCY_SLOTS]);
    console.log(`Registered worker in database: ${WORKER_ID}`);
}
async function sendHeartbeat() {
    try {
        await (0, database_1.query)('UPDATE workers SET last_heartbeat_at = NOW(), status = "active" WHERE id = ?', [WORKER_ID]);
        (0, eventBus_1.emitEvent)('worker:heartbeat', { id: WORKER_ID, last_heartbeat_at: new Date() });
    }
    catch (error) {
...
async function unregisterWorker() {
    try {
        await (0, database_1.query)('UPDATE workers SET status = "offline", updated_at = NOW() WHERE id = ?', [WORKER_ID]);
...
async function claimJobs(limit) {
    ...
    const activeQueues = await (0, database_1.query)('SELECT id FROM queues WHERE is_paused = 0');
    ...
    `;
        ...
        for (const job of jobs) {
            ...
            await conn.query('UPDATE jobs SET status = "claimed", last_execution_id = ?, updated_at = NOW() WHERE id = ?', [execId, job.id]);
            await conn.query(`INSERT INTO job_executions (id, job_id, worker_id, status, started_at, retry_number)
         VALUES (?, ?, ?, 'running', NOW(), ?)`, [execId, job.id, WORKER_ID, job.retry_count]);
            claimedJobs.push({
                ...job,
                last_execution_id: execId
            });
            (0, eventBus_1.emitEvent)('job:updated', { id: job.id, status: 'claimed' });
        }
        await conn.commit();
        conn.release();
        ...
        const policyRows = await (0, database_1.query)(`
    ...
    `, claimedQueueIds);
        const policyMap = new Map(policyRows.map((r) => [r.queue_id, r]));
...
async function runJob(job) {
    ...
    try {
        await (0, database_1.query)('UPDATE jobs SET status = "running", updated_at = NOW() WHERE id = ?', [job.id]);
        (0, eventBus_1.emitEvent)('job:updated', { id: job.id, status: 'running' });
    ...
    try {
        ...
        if (jobNameLower.includes('email')) {
            ...
            stdoutLogs += `[${new Date().toISOString()}] Email successfully dispatched message-id: ${(0, crypto_1.randomUUID)()}\n`;
        }
            ...
            stdoutLogs += `[${new Date().toISOString()}] PDF Report written to static s3://acme-bucket/reports/rep-${(0, crypto_1.randomUUID)().substring(0, 8)}.pdf\n`;
        }
...
async function finalizeJob(job, execId, status, durationMs, stdoutLogs, stderrLogs, errorMessage) {
    const conn = await (0, database_1.getTransaction)();
    try {
        // 1. Update job_executions record
        await conn.query(`UPDATE job_executions 
       SET status = ?, finished_at = NOW(), duration_ms = ?, stdout_logs = ?, stderr_logs = ?, error_message = ?
       WHERE id = ?`, [status, durationMs, stdoutLogs, stderrLogs, errorMessage || null, execId]);
        if (status === 'completed') {
            // Job completed successfully
            await conn.query('UPDATE jobs SET status = "completed", updated_at = NOW() WHERE id = ?', [job.id]);
            await conn.commit();
            conn.release();
            console.log(`[Worker] Job completed: "${job.name}" (${job.id}) in ${durationMs}ms`);
            (0, eventBus_1.emitEvent)('job:updated', { id: job.id, status: 'completed' });
        }
        else {
            ...
            if (nextRetryCount <= job.max_retries) {
                ...
                await conn.query('UPDATE jobs SET status = "queued", retry_count = ?, run_at = ?, updated_at = NOW() WHERE id = ?', [nextRetryCount, runAt, job.id]);
                await conn.commit();
                conn.release();
                console.log(`[Worker] Job "${job.name}" failed, scheduling retry #${nextRetryCount} in ${delayMs}ms (at ${runAt.toISOString()})`);
                (0, eventBus_1.emitEvent)('job:updated', { id: job.id, status: 'queued', retry_count: nextRetryCount, run_at: runAt });
            }
            else {
                // Permanent failure -> Move to Dead Letter Queue (DLQ)
                await conn.query('UPDATE jobs SET status = "dlq", updated_at = NOW() WHERE id = ?', [job.id]);
                const dlqId = (0, crypto_1.randomUUID)();
                await conn.query(`INSERT INTO dead_letter_queue (id, job_id, queue_id, failed_at, error_summary, original_payload, retry_count)
           VALUES (?, ?, ?, NOW(), ?, ?, ?)`, [
                    dlqId,
                    job.id,
                    job.queue_id,
                ...
                (0, eventBus_1.emitEvent)('job:updated', { id: job.id, status: 'dlq' });
                (0, eventBus_1.emitEvent)('dlq:new', { id: dlqId, job_id: job.id, error_summary: errorMessage });
            }
        }
    }
    catch (error) {
...
async function recoverOrphanedJobs() {
    try {
        ...
        const deadWorkers = await (0, database_1.query)('SELECT id, hostname FROM workers WHERE status = "active" AND last_heartbeat_at < ?', [threshold]);
        for (const worker of deadWorkers) {
            ...
            await (0, database_1.query)('UPDATE workers SET status = "offline", updated_at = NOW() WHERE id = ?', [worker.id]);
            // Find jobs locked by this worker that are in claimed or running status
            const orphanedJobs = await (0, database_1.query)(`SELECT j.id, j.name, j.retry_count, j.max_retries, j.queue_id, j.payload
         FROM jobs j
         JOIN job_executions je ON j.last_execution_id = je.id
         WHERE je.worker_id = ? AND j.status IN ('claimed', 'running')`, [worker.id]);
            for (const job of orphanedJobs) {
                ...
                try {
                    ...
                    await conn.query(`UPDATE job_executions je
             ...
             WHERE je.job_id = ? AND je.worker_id = ? AND je.status = 'running'`, [errorMsg, job.id, worker.id]);
                    const nextRetry = job.retry_count + 1;
                    if (nextRetry <= job.max_retries) {
                        // Re-queue the job immediately
                        await conn.query('UPDATE jobs SET status = "queued", retry_count = ?, run_at = NOW(), updated_at = NOW() WHERE id = ?', [nextRetry, job.id]);
                        console.log(`[Recovery Coordinator] Re-queued job "${job.name}" (${job.id})`);
                        (0, eventBus_1.emitEvent)('job:updated', { id: job.id, status: 'queued', retry_count: nextRetry });
                    }
                    else {
                        // Move to DLQ
                        await conn.query('UPDATE jobs SET status = "dlq", updated_at = NOW() WHERE id = ?', [job.id]);
