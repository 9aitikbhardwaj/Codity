...
async function claimJobsMock(workerId, limit) {
    ...
    `;
        ...
        for (const job of jobs) {
         ...
         VALUES (?, ?, ?, 'running', NOW(), ?)`, [execId, job.id, workerId, job.retry_count]);
...
async function recoverOrphanedJobsMock() {
    ...
    const deadWorkers = await (0, database_1.query)('SELECT id FROM workers WHERE status = "active" AND last_heartbeat_at < ?', [threshold]);
    for (const worker of deadWorkers) {
        await (0, database_1.query)('UPDATE workers SET status = "offline" WHERE id = ?', [worker.id]);
        const orphanedJobs = await (0, database_1.query)(`SELECT j.id, j.name, j.retry_count, j.max_retries, j.queue_id
       FROM jobs j
       ...
       WHERE je.worker_id = ? AND j.status IN ('claimed', 'running')`, [worker.id]);
        for (const job of orphanedJobs) {
           ...
           WHERE je.job_id = ? AND je.worker_id = ? AND je.status = 'running'`, [job.id, worker.id]);
                const nextRetry = job.retry_count + 1;
                if (nextRetry <= job.max_retries) {
                    await conn.query('UPDATE jobs SET status = "queued", retry_count = ?, run_at = NOW() WHERE id = ?', [nextRetry, job.id]);
                }
                else {
             ...
             VALUES (?, ?, ?, NOW(), 'Exceeded retries', '{}', ?)`, [(0, crypto_1.randomUUID)(), job.id, job.queue_id, job.retry_count]);
                }
                await conn.commit();
            }
            catch (err) {
...
async function runTests() {
    ...
    try {
        // Cleanup tables
        await (0, database_1.query)('DELETE FROM dead_letter_queue');
        await (0, database_1.query)('DELETE FROM job_executions');
        await (0, database_1.query)('DELETE FROM jobs');
        await (0, database_1.query)('DELETE FROM workers');
        console.log('✔ Cleaned up tables for testing');
        // Make sure we have a queue
        const queues = await (0, database_1.query)('SELECT id FROM queues LIMIT 1');
        if (queues.length === 0) {
        ...
        for (let i = 0; i < 5; i++) {
            ...
            jobIds.push(id);
            await (0, database_1.query)(`INSERT INTO jobs (id, queue_id, name, payload, status, run_at, retry_count, max_retries) 
         VALUES (?, ?, ?, '{}', 'queued', NOW(), 0, 3)`, [id, testQueueId, `Concurrency Job ${i}`]);
        }
        // Register 3 workers
        const w1 = 'test-worker-1';
        const w2 = 'test-worker-2';
        const w3 = 'test-worker-3';
        await (0, database_1.query)('INSERT INTO workers (id, hostname, ip_address, status) VALUES (?, "W1", "127.0.0.1", "active")', [w1]);
        await (0, database_1.query)('INSERT INTO workers (id, hostname, ip_address, status) VALUES (?, "W2", "127.0.0.1", "active")', [w2]);
        await (0, database_1.query)('INSERT INTO workers (id, hostname, ip_address, status) VALUES (?, "W3", "127.0.0.1", "active")', [w3]);
        ...
        await (0, database_1.query)('UPDATE workers SET last_heartbeat_at = ? WHERE id = ?', [thirtySecsAgo, w3]);
        ...
        await (0, database_1.query)('UPDATE jobs SET status = "running" WHERE id = ?', [jobOfW3.id]);
        // Execute recovery coordinator
        await recoverOrphanedJobsMock();
        // Verify Worker 3 is offline
        const w3Status = await (0, database_1.query)('SELECT status FROM workers WHERE id = ?', [w3]);
        ...
        const jobStatus = await (0, database_1.query)('SELECT status, retry_count FROM jobs WHERE id = ?', [jobOfW3.id]);
        ...
        const execLog = await (0, database_1.query)('SELECT status, error_message FROM job_executions WHERE job_id = ? AND worker_id = ?', [jobOfW3.id, w3]);
